...
@section('content')
    <div class="row m-4">
        <div class="bg-light rounded h-100 p-4">
            ...
            <form action="{{route('discount.update',$offer->id)}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
               ...
               {{-- <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="price" id="price" value="{{$offer->discount}}">
                </div>
                <div class="mb-3">
                    ...
                    <select type="text" class="form-control" name="service_category" id="service_category">
                        @foreach($offer_category as $category)
                            <option value="{{$category->id}}" {{$category->id == $offer->service_category->id ? "selected" : ""}}>{{$category->name}}</option>
                        @endforeach
                    </select>
                </div>
